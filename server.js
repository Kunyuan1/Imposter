...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (data) => {
    ...
    if (msg.type === "create") {
      ...
      currentPlayer = { name: msg.name, ws };
      currentRoom = {
        code,
        host: msg.name,
    ...
    // SUBMIT CLUE
      ...
      if (!currentRoom) return;
      const clue = { player: currentPlayer.name, clue: msg.clue };
      currentRoom.clues.push(clue);
      const nextIndex = currentRoom.currentPlayerIndex + 1;

      if (nextIndex >= currentRoom.players.length) {
        currentRoom.currentPlayerIndex = 0;
        currentRoom.phase = "majorityVote";
        broadcast(currentRoom, {
          type: "phase_change",
          phase: "majorityVote",
        ...
        currentRoom.currentPlayerIndex = nextIndex;
        broadcast(currentRoom, {
          type: "phase_change",
          phase: "clue",
